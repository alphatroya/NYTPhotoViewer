...
- (instancetype)initWithPhoto:(id <NYTPhoto>)photo loadingView:(UIView *)loadingView notificationCenter:(NSNotificationCenter *)notificationCenter {
    ...
    if (self) {
        ...
        if ([photo conformsToProtocol:@protocol(NYTURLPhoto)]) {
            id <NYTURLPhoto> urlPhoto = (id <NYTURLPhoto>) photo;
            _scalingImageView = [[NYTScalingImageView alloc] initWithURL:urlPhoto.photoURL frame:CGRectZero placeholderImage:urlPhoto.placeholderImage completedBlock:^(UIImage *image, NSError *error) {
                [self updateImage:image];
            }];
        } else {
            _scalingImageView = [[NYTScalingImageView alloc] initWithImage:photo.image ?: photo.placeholderImage frame:CGRectZero];
        }
